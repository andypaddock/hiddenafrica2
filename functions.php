<?php
/**
 * hiddenafrica functions and definitions
 *
...
add_action('wp_enqueue_scripts', 'hiddenafrica_scripts');

/* Add Menus */
add_action('init', 'hiddenafrica_custom_menu');

/* Dashboard Config */
add_action('wp_dashboard_setup', 'hiddenafrica_dashboard_widget');

/* Dashboard Style */
add_action('admin_head', 'hiddenafrica_custom_fonts');

/* Remove Default Menu Items */
add_action('admin_menu', 'hiddenafrica_remove_menus');

/* Change Posts Columns */
add_filter('manage_posts_columns', 'hiddenafrica_manage_columns');

/* Reorder Admin Menu */
add_filter('custom_menu_order', 'hiddenafrica_reorder_menu');
add_filter('menu_order', 'hiddenafrica_reorder_menu');
...
function hiddenafrica_scripts() {
	wp_enqueue_style( 'hiddenafrica-style', get_template_directory_uri() . '/style.css', array(), filemtime(get_template_directory() . '/style.css'), false);
	wp_enqueue_script( 'hiddenafrica-core-js', get_template_directory_uri() . '/inc/js/compiled.js', array('jquery'), filemtime( get_stylesheet_directory() . '/inc/js/compiled.js' ), true);
}

add_filter( 'script_loader_tag', function ( $tag, $handle ) {

	if ( 'hiddenafrica-intersection-js' !== $handle )
		return $tag;

	return str_replace( ' src', ' defer="defer" src', $tag );
}, 10, 2 );

function hiddenafrica_custom_menu() {
...
function hiddenafrica_dashboard_widget() {
	global $wp_meta_boxes;
	wp_add_dashboard_widget('custom_help_widget', 'hiddenafrica Support', 'hiddenafrica_dashboard_help');
}

function hiddenafrica_dashboard_help() {
	echo file_get_contents(__DIR__ . "/admin-settings/dashboard.html");
}

function hiddenafrica_custom_fonts() {
...
function hiddenafrica_remove_menus(){
	remove_menu_page( 'edit-comments.php' ); //Comments
}

function hiddenafrica_manage_columns($columns) {
	unset($columns["comments"]);
	return $columns;
}

function hiddenafrica_reorder_menu() {
    return array(
		'index.php',                        // Dashboard
		'separator1',                       // --Space--
		'edit.php',                         // Posts
...
function hiddenafrica_manage_admin_bar(){
	global $wp_admin_bar;
